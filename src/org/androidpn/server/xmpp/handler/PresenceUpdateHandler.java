...
import org.androidpn.server.message.MessageManager;
        ...
        try {
            ...
            if (type == null) { // null == available
                ...
                if (session != null) {
                    ...
                    List<Notification> notifiactionByUserName = notificationService.getNotifiactionByUserName(session.getUsername());
                    
                    if (notifiactionByUserName != null && notifiactionByUserName.size() > 0) {
                    	for (Notification notification : notifiactionByUserName) {
                    		...
                    		// 这里发送,是数据库已经存在数据的推送,不需要在推送后存库,所有是false
                    		notificationManager.sendNotifcationToUser(apiKey, username, title, message, uri, uuid);
                    	}
                    }
                    List<MessageIM> findMessageByUsername = ServiceLocator.getMessageService().findMessageByUsername(session.getUsername());
                    if (notifiactionByUserName != null && notifiactionByUserName.size() > 0) {
                    	for (MessageIM messageIM : findMessageByUsername) {
							Packet message = messageIM.toXML();
							messageManager.sendMessageIM(session.getUsername(), message);
						}
                    }
